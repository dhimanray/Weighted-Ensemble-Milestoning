...
class ODLDPropagator(WESTPropagator):
    def __init__(self, rc=None):
        ...
        self.initial_pcoord = numpy.array([-2.0], dtype=self.coord_dtype)  #Initial Position in x coordinate
        
        self.sigma = 0.001**(0.5)   #standard daviation of random noise
    ...
    def propagate(self, segments):
        ...
        for istep in xrange(1,coord_len):
            ...
            newx[not_to_absorb] = x[not_to_absorb] - gradfactor*grad[not_to_absorb] + displacements[not_to_absorb]
...
class ODLDSystem(WESTSystem):
	...
	self.bin_mapper = RectilinearBinMapper([ list(numpy.arange(-3.0, 3.0, 0.1)) ])  #bins for propagation
