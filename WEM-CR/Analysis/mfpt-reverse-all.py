...
f2 = open('mfpt-reverse-all.dat','w')
print >>f2, '#starting_distance #MFPT #standard_deviation #95%_uncertainty'

#loop over milestones
for Ns in range(2,Nend):
    ...
    print >>f2, mps[Ns-1], np.mean(b), np.std(b), 1.96*np.sqrt(np.std(b)**2/counter)
