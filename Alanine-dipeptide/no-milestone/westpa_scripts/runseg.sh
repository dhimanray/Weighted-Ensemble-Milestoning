...
ln -sv $WEST_SIM_ROOT/namd_config/A.psf structure.psf
ln -sv $WEST_SIM_ROOT/namd_config/A.pdb structure.pdb 
ln -sv $WEST_SIM_ROOT/namd_config/toppar .

#Make symbolic lik to colvars input file
ln -sv $WEST_SIM_ROOT/bstates/unbound/dihedral.in dihedral.in
...
ln -sv $WEST_PARENT_DATA_REF/psi.dat  ./parent_psi.dat

############################## Run the dynamics ################################
# Propagate the segment using namd2 
/data/users/dray1/NAMD_2.12_Linux-x86_64-multicore/namd2 md.conf > seg.log 
...
python $WEST_SIM_ROOT/westpa_scripts/calculate_psi.py > psi.dat 
cat psi.dat > $WEST_PCOORD_RETURN
...
# Clean up
rm -f seg.dcd seg.log md.conf parent.coor parent.dcd parent.vel parent.xsc seg.pdb \
