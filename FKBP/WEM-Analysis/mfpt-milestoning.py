...
mps0 = [ 5.0 ]
mps = mps0 + mps1
#N = 11  #The target (product) milestone (index starts from 1).  
N = len(mps)

#starting milestone (reactant or bound state) (index starts from 1)
Ns = 2

#target milestone (product or unbound state) (index starts from 1)
Nend = 7 #r = 16.0 A
...
print >>f2, '#iteration #MFPT #standard_deviation #95%_uncertainty'
...
for k in range(N_total):
	...
	print >>f2, k+1, np.mean(b), np.std(b), 1.96*np.sqrt(np.std(b)**2/counter)
